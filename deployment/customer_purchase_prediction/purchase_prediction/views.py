...
def purchase_prediction_view(request):
    ...
    def bar_n_unique_customer_plot():
        engine = sqlite3.connect(os.path.join(CURDIR,
                                              'db.sqlite3'
                                              )
                                 )
        query = """
                select
                product_details.category as product_category,
                count(distinct purchase_history.customer_id) as n_customer
                from purchase_history
                left join product_details on purchase_history.product_id = product_details.product_id
                group by 1
                """
        ...
        layout = {
            'title': 'Purchase by Product Category',
            'yaxis': {'range': [data.n_customer.min(), data.n_customer.max()]},
            'xaxis': {'categoryarray': data.product_category.unique().tolist()}
            # 'xaxis':
        }
        fig = go.Figure(data=[bar_],
        ...
        fig.update_layout(barmode='stack',
                          title_text='Numberof Unique Customer',
                          xaxis_title='Product Category',
                          yaxis_title='Number of Unique Customer'
                          )
        plot_fig = po(fig,
                      output_type='div',
    ...
    context = {
        ...
        'product_historical_uniquecustomer_bar': bar_n_unique_customer_plot()
    }
